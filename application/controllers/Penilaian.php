    ...
    public function index()
{
    ...
    {
        $all_id = $this->input->post('all_kegiatan_pencacah_id');
        $nilai_array = $this->input->post('nilai'); // [kriteria_id => nilai]

        foreach ($nilai_array as $kriteria_id => $nilai) {
            // Validasi jika perlu: pastikan nilai di antara 0-100
            $nilai = max(0, min(100, (int)$nilai));
            
            // Simpan nilai - update atau insert tergantung kebutuhan
