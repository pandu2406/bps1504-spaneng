...
class Penilaian extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($data['user']['role_id'] == 1) {
            // ADMIN VIEW: Global Summary of all Mitra

            $filter_bulan = $this->input->get('bulan');
            $filter_tahun = $this->input->get('tahun');
            $search_nama = $this->input->get('nama');

            $where_clauses = [];

            if ($filter_bulan) {
                $where_clauses[] = "FROM_UNIXTIME(k.finish, '%m') = '$filter_bulan'";
            }
            if ($filter_tahun) {
                $where_clauses[] = "FROM_UNIXTIME(k.finish, '%Y') = '$filter_tahun'";
            }
            if ($search_nama) {
                $where_clauses[] = "m.nama LIKE '%$search_nama%'";
            }
            ...
            $query_summary = "
                ...
                FROM mitra m
                LEFT JOIN all_kegiatan_pencacah akp ON m.id_mitra = akp.id_mitra
                LEFT JOIN kegiatan k ON akp.kegiatan_id = k.id
                LEFT JOIN all_penilaian p ON akp.id = p.all_kegiatan_pencacah_id
                $where_sql
                GROUP BY m.id_mitra
                ORDER BY rata_rata DESC, total_kegiatan DESC
            ";
            $data['mitra_summary'] = $this->db->query($query_summary)->result_array();
            ...
            $this->load->view('penilaian/summary', $data);
    ...
    ";
        ...
        foreach ($data['kegiatan'] as &$k) {
            // Get all assigned mitras for this supervisor in this activity
            $mitras = $this->db->select('id')
                ->from('all_kegiatan_pencacah')
                ->where('kegiatan_id', $k['id'])
                ->where('id_pengawas', $id_peg)
                ->get()->result_array();

            $k['total_mitra'] = count($mitras);
            $k['assessed_mitra'] = 0;

            if ($k['total_mitra'] > 0) {
                $ids = array_column($mitras, 'id');
                // Count assessed criteria for each mitra
                $this->db->select('all_kegiatan_pencacah_id, count(*) as cnt');
                $this->db->from('all_penilaian');
                $this->db->where_in('all_kegiatan_pencacah_id', $ids);
                $this->db->group_by('all_kegiatan_pencacah_id');
                $assessed_counts = $this->db->get()->result_array();

                foreach ($assessed_counts as $ac) {
                    if ($ac['cnt'] >= $jumlah_kriteria) {
                        $k['assessed_mitra']++;
                    }
                }
            }
    ...
    public function index_pml()
    {
        ...
        // Pass selections to view
        $data['selected_seksi'] = $seksi_id;
        $data['selected_bulan'] = $bulan;
        $data['selected_tahun'] = $tahun;

        // Get list of Seksi for dropdown
        $data['seksi_list'] = $this->db->get('seksi')->result_array();
    ...
    ";
        ...
        if ($bulan && $tahun) {
            // Construct timestamp range for the selected month
            // Start of month: YYYY-MM-01 00:00:00
            $start_date_str = "$tahun-$bulan-01 00:00:00";
            $start_ts = strtotime($start_date_str);

            // End of month
            $end_date_str = date('Y-m-t 23:59:59', $start_ts);
            $end_ts = strtotime($end_date_str);

            // Logic: Activity overlaps with the selected month
            // Activity Start <= Month End AND Activity Finish >= Month Start
            $this->db->group_start();
            $this->db->where('kegiatan.start <=', $end_ts);
            $this->db->where('kegiatan.finish >=', $start_ts);
            $this->db->group_end();
        } elseif ($tahun) {
            // If only year is selected
            $start_year_ts = strtotime("$tahun-01-01 00:00:00");
            $end_year_ts = strtotime("$tahun-12-31 23:59:59");

            $this->db->group_start();
            $this->db->where('kegiatan.start <=', $end_year_ts);
            $this->db->where('kegiatan.finish >=', $start_year_ts);
            $this->db->group_end();
        }
        ...
        foreach ($data['kegiatan'] as &$k) {
            ...
            if ($data['id_peg']) {
                $this->db->where('id_pengawas', $data['id_peg']);
            }
    ...
    public function daftar_pencacah($kegiatan_id, $id_peg)
    {
        ...
        $jumlah_kriteria = $this->db->count_all('kriteria');
        foreach ($data['kegiatan'] as &$pencacah) {
            $cnt = $this->db->where('all_kegiatan_pencacah_id', $pencacah['id'])->count_all_results('all_penilaian');
            $pencacah['is_full'] = ($cnt >= $jumlah_kriteria);
            $pencacah['progress_count'] = $cnt;
            $pencacah['total_kriteria'] = $jumlah_kriteria;
        }
        unset($pencacah);
    ...
    ";
        ...
        // Cek status penilaian pengawas
        $jumlah_kriteria = $this->db->count_all('kriteria');
        foreach ($data['pengawas'] as &$p) {
            $cnt = $this->db->where('all_kegiatan_pengawas_id', $p['id'])->count_all_results('all_penilaian_pengawas');
            $p['is_full'] = ($cnt >= $jumlah_kriteria);
            $p['progress_count'] = $cnt;
            $p['total_kriteria'] = $jumlah_kriteria;
        }
        unset($p);
    ...
    public function isi_nilai($kegiatan_id, $id_peg, $id_target, $peran = 'mitra')
    {
        ...
        if ($peran == 'mitra') {
            ...
            $data['all_kegiatan_pengawas'] = $this->db->get_where('all_kegiatan_pengawas', $where)->row_array();
