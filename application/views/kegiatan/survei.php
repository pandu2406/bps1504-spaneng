    ...
    <div class="col-lg">
      ...
      <div class="card shadow mb-4">
        <div class="card-header py-3 bg-primary">
          <h6 class="m-0 font-weight-bold text-white"><i class="fas fa-filter mr-2"></i>Filter Kegiatan Survei</h6>
        </div>
        <div class="card-body">
          <form action="<?= base_url('kegiatan/survei'); ?>" method="get">
            <div class="row align-items-end">
              ...
              <div class="col-md-4">
                <div class="form-group mb-0">
                  ...
                  <a href="<?= base_url('kegiatan/survei'); ?>"
      ...
      <a href="" class="btn btn-primary mb-3 shadow-sm" data-toggle="modal" data-target="#newSurveiModal"><i
          class="fas fa-plus fa-sm mr-1"></i>Add New Survei</a>
        ...
        <table class="table table-borderless table-hover" id="mydata">
          <thead style="background-color: #00264d; color:#e6e6e6;">
            <tr align="center">
              ...
              <th scope="col">Penanggung Jawab</th> <!-- Kolom baru -->
          ...
          <tbody style="background-color: #ffffff; color: #00264d;">
            ...
            <?php foreach ($survei as $s): ?>
              <tr align="center">
                <th scope="row"><?= $i++; ?></th>
                <td><?= $s['nama']; ?></td>
                <td><?= $s['periodisitas']; ?></td>
                <td><?= $s['nama_seksi'] ?? '<i class="text-muted">Belum ada</i>'; ?></td> <!-- Menampilkan nama seksi -->
                ...
                <td>
                  ...
                  <a href="javascript:void(0);" class="badge badge-danger btn-delete-survei"
                    data-url="<?= base_url('kegiatan/deletesurvei/' . $s['id']); ?>"
...
<!-- Modal Add New Survei -->
<div class="modal fade" id="newSurveiModal" tabindex="-1" role="dialog" aria-labelledby="newSurveiModalLabel"
  aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document"> <!-- Changed to modal-lg -->
    <div class="modal-content">
      <div class="modal-header bg-primary text-white">
        <h5 class="modal-title" id="newSurveiModalLabel"><i class="fas fa-plus-circle mr-2"></i>Tambah Survei Baru</h5>
        <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form action="<?= base_url('kegiatan/survei'); ?>" method="post" autocomplete="off">
            ...
            <div class="col-md-6 border-right">
              ...
              <div class="form-group">
                ...
                <div class="input-group">
                  ...
                  <input type="text" class="form-control" name="nama" placeholder="Ketik Nama Kegiatan" required>
                </div>
              </div>
...
<!-- Robust SweetAlert Script for Delete Survei (Vanilla JS) -->
<script>
  document.addEventListener('DOMContentLoaded', function () {
    // Event delegation using Vanilla JS
    document.body.addEventListener('click', function (e) {
      // Check if clicked element is .btn-delete-survei or is inside it (e.g. icon)
      const btn = e.target.closest('.btn-delete-survei');
      ...
      if (btn) {
        ...
        // Ensure Swal is defined (SweetAlert loaded)
        if (typeof Swal !== 'undefined') {
          Swal.fire({
            title: 'Konfirmasi Hapus',
            html: `
              <p>Anda akan menghapus kegiatan:</p>
              <h4 style="color: #4e73df; font-weight: bold; margin: 10px 0;">${nama}</h4>
  ...
  // If the user had typed something (new tag) or selected something that matches the filter, restore it
  if (currentVal && filtered.some(i => i.nama === currentVal)) {
    $select.val(currentVal);
  }

  // Trigger update to refresh Select2 UI
  $select.trigger('change');

            // Re-open if needed or focus? No, might be annoying.
        }

  // 4. Event Listener on Periodisitas
  $('#periodisitas').on('change', function () {
    const selectedPeriodicity = $(this).val();
    if (selectedPeriodicity) {
      filterActivities(selectedPeriodicity);
    } else {
      // Clear if no periodicity selected
      const $select = $('#nama_kegiatan');
