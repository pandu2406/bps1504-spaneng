<div class="container-fluid mb-5">
    ...
    <!-- Page Header -->
    <h1 class="h3 mb-4 text-gray-800 font-weight-bold">Entry Laporan Perjalanan Dinas (Mitra)</h1>
    ...
    <form id="formLPD" action="<?= base_url('persuratan/save_lpd_mitra'); ?>" method="post"
        enctype="multipart/form-data">

        <div class="row">
            <!-- Left Column: Informasi Mitra & Tugas -->
            <div class="col-lg-6 mb-4">

                <!-- Card Informasi Mitra -->
                <div class="card shadow mb-4">
                    <div class="card-header py-3 bg-gradient-success text-white">
                        <h6 class="m-0 font-weight-bold"><i class="fas fa-users mr-2"></i>Informasi Mitra Statistik</h6>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <label class="small font-weight-bold text-secondary">Nama Mitra</label>
                            <select name="nama" id="nama_mitra" class="form-control select2" required
                                onchange="isiMitra()">
                        ...
                        <div class="form-group">
                            <label class="small font-weight-bold text-secondary">ID Sobat</label>
                            <input type="text" name="nip" id="id_sobat" class="form-control bg-light" readonly
                                placeholder="ID Sobat otomatis..." required>
                        </div>
                    </div>
                </div>
                ...
                <div class="card shadow">
                    <div class="card-header py-3 bg-white border-left-primary">
                        <h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-briefcase mr-2"></i>Detail
...
<script>const CACHE_KEY = 'formLPD_mitra_cache';
    ...
    function isiMitra() {
        // Safe access (in case select2 not fully init or empty)
        const select = $('#nama_mitra');

        if (select.val()) {
            const selected = select.select2('data')[0].element;
            const idSobat = $(selected).data('idsobat');
            $('#id_sobat').val(idSobat);
        }
    ...
    oleh mitra atas nama $ {
        nama
    }

    . Kegiatan ini merupakan bagian dari pelaksanaan tugas berdasarkan surat tugas yang berlaku.\n\nSelama pelaksanaan kegiatan,
    mitra melakukan aktivitas seperti $ {
        kegiatanUnik
    }

    . Kegiatan diawali dengan koordinasi di lokasi,
    kemudian dilanjutkan dengan pelaksanaan dan pencatatan sesuai rundown yang telah direncanakan.\n\nSecara umum kegiatan berjalan dengan baik. Jika terdapat kendala teknis di lapangan,
    mitra melakukan koordinasi dengan pihak terkait untuk penyelesaian lebih lanjut.`;
    ...
    function loadCachedForm() {
        ...
        // Select2
        if (data['nama']) $('#nama_mitra').val(data['nama']).trigger('change');
        setTimeout(isiMitra, 200);
