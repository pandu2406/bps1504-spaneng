...
$username = 'root';
$password = '';
$database = 'spaneng_test'; // CORRECT DATABASE
...
if ($conn->query($sql_period) === TRUE) {
    echo "Table master_periodisitas created successfully in $database\n";
} else {
...
if ($conn->query($sql_log) === TRUE) {
    echo "Table traffic_log created successfully in $database\n";
} else {
    echo "Error creating traffic_log: " . $conn->error . "\n";
}

// 4. Drop unused table (in correct DB this time)
